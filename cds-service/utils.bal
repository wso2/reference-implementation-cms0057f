...
isolated function preProcessing(string hookId, cds:CdsRequest cdsRequest) returns cds:CdsRequest|http:Response {
    ...
    lock {
        ...
        if (cds_services.filter(s => s.id == hookId).length() > 0) {
            cds:CdsService cdsService = <cds:CdsService>cds_services.filter(s => s.id == hookId)[0];

            cds:CdsRequest|constraint:Error validate = constraint:validate(cdsRequest.clone(), cds:CdsRequest);
            ...
            cds:CdsError? contextValidated = cds:validateContext(cdsRequest.clone(), cdsService);
            if contextValidated is cds:CdsError {
                return cds:cdsErrorToHttpResponse(contextValidated);
            }

            //Do Prefetch FHIR data validation
            cds:CdsRequest|cds:CdsError prefetchValidated = cds:validateAndProcessPrefetch(cdsRequest.clone(), cdsService);
            if prefetchValidated is cds:CdsError {
                return cds:cdsErrorToHttpResponse(prefetchValidated);
            }

            return prefetchValidated.clone();
        } else {
