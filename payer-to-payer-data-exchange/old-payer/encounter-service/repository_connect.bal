...
public isolated function create(international401:Encounter payload) returns r4:FHIRError|r4:FHIRParseError? {
...
public isolated function getById(string id) returns r4:FHIRError|international401:Encounter {
    lock {
...
public isolated function search(map<string[]>? searchParameters = ()) returns r4:FHIRError|r4:Bundle {
    ...
    if searchParameters is map<string[]> {
        ...
        foreach var 'key in searchParameters.keys() {
            match 'key {
                "_id" => {
                    international401:Encounter byId = check getById(searchParameters.get('key)[0]);
                    bundle.entry = [
                        {
                            'resource: byId
                        }
