...
public isolated function createEncounter(json payload) returns r4:FHIRError|uscore700:USCoreEncounterProfile {
...
public isolated function getByIdEncounter(string id) returns r4:FHIRError|uscore700:USCoreEncounterProfile {
    lock {
...
public isolated function searchEncounter(map<string[]>? searchParameters = ()) returns r4:FHIRError|r4:Bundle {
    ...
    if searchParameters is map<string[]> {
        foreach var 'key in searchParameters.keys() {
            match 'key {
                "_id" => {
                    uscore700:USCoreEncounterProfile byId = check getByIdEncounter(searchParameters.get('key)[0]);
                    bundle.entry = [
                        {
                            'resource: byId
                        }
