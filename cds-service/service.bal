...
// Adding due to issue in Choreo loading configs
final isolated cds:CdsService[] cds_services = [
    {
        id: "crd-mri-spine-order-sign",
        hook: "order-sign",
        title: "CRD - MRI Spine (Prior Auth + Medical Necessity + Docs)",
    ...
    },
    {
        ...
        hook: "order-sign",
        title: "Medication Prescribe Hook",
        description: "Checks coverage requirements for medication orders.",
        usageRequirements: "Requires context.patientId and context.draftOrders.",
        prefetch: {
            patient: "Patient/{{context.patientId}}"
        }
    }
...
isolated function submitForDecision(string hookId, cds:CdsRequest cdsRequest) returns cds:CdsResponse|cds:CdsError {
    log:printInfo("Received hookId: " + hookId);
    match (hookId) {
        "crd-mri-spine-order-sign" => {
...
isolated function submitFeedback(string hookId, cds:Feedbacks feedback) returns cds:CdsError? {
    match (hookId) {
        "crd-mri-spine-order-sign" => {
            return connectFeedbackSystemForCrdMriSpineOrderSign(feedback, hookId);
        }
        "prescribe-medication" => {
